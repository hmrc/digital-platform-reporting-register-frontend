...
class CanPhonePrimaryContactController @Inject()(sessionRepository: SessionRepository,
  ...
  def onPageLoad(mode: Mode): Action[AnyContent] = (identify andThen getData andThen requireData) { implicit request =>
    ...
    Ok(view(CanPhonePrimaryContactViewModel(mode, userAnswers, formProvider())))
  }

  def onSubmit(mode: Mode): Action[AnyContent] = (identify andThen getData andThen requireData).async { implicit request =>
    formProvider().bindFromRequest().fold(
      formWithErrors => Future.successful(BadRequest(view(CanPhonePrimaryContactViewModel(mode, request.userAnswers, formWithErrors)))),
      value =>
        for {
