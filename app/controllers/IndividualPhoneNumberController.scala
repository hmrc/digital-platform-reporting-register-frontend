...
class IndividualPhoneNumberController @Inject()(sessionRepository: SessionRepository,
  ...
  def onPageLoad(mode: Mode): Action[AnyContent] = (identify andThen getData andThen requireData) { implicit request =>
    ...
    Ok(view(IndividualPhoneNumberViewModel(mode, userAnswers, formProvider())))
  }

  def onSubmit(mode: Mode): Action[AnyContent] = (identify andThen getData andThen requireData).async { implicit request =>
    formProvider().bindFromRequest().fold(
      formWithErrors => Future.successful(BadRequest(view(IndividualPhoneNumberViewModel(mode, request.userAnswers, formWithErrors)))),
      value =>
        for {
          updatedAnswers <- Future.fromTry(request.userAnswers.set(IndividualPhoneNumberPage, value))
          _ <- sessionRepository.set(updatedAnswers)
        } yield Redirect(IndividualPhoneNumberPage.nextPage(mode, updatedAnswers))
    )
  }
}
