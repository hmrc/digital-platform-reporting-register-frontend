...
class BusinessNameNoUtrController @Inject()(sessionRepository: SessionRepository,
  ...
  def onSubmit(mode: Mode): Action[AnyContent] = (identify andThen getData andThen requireData).async { implicit request =>
    formProvider().bindFromRequest().fold(
      ...
      value =>
        for {
          updatedAnswers <- Future.fromTry(request.userAnswers.set(BusinessNameNoUtrPage, value))
          _ <- sessionRepository.set(updatedAnswers)
        } yield Redirect(BusinessNameNoUtrPage.nextPage(mode, updatedAnswers))
    )
  }
}
