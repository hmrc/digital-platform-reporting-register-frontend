...
import models.BusinessType.{AssociationOrTrust, Individual, LimitedCompany}
...
case object BusinessTypePage extends QuestionPage[BusinessType] {
  ...
  override protected def nextPageNormalMode(answers: UserAnswers): Call = answers.get(this).map {
    ...
    case LimitedCompany | AssociationOrTrust if answers.user.isIndividualAffinityGroup =>
      routes.CannotUseServiceThirdPartyIndividualController.onPageLoad()
