...
class ClaimEnrolmentFormProvider @Inject() extends Mappings {

  def apply(): Form[ClaimEnrolmentDetails] = Form(
    mapping(
      "utr" -> text("claimEnrolment.error.utr.required")
        .verifying(regexp(Validation.utrPattern.toString, "claimEnrolment.error.utr.format")
        ),
      "businessName" -> text("claimEnrolment.error.businessName.required")
