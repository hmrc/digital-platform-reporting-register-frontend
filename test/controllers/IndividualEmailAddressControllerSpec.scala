...
class IndividualEmailAddressControllerSpec extends ControllerSpecBase with MockitoSugar {
  ...
  "IndividualEmailAddress Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      val userAnswers = anEmptyAnswer.set(IndividualEmailAddressPage, "foo@example.com").success.value
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        ...
        redirectLocation(result).value mustEqual IndividualEmailAddressPage.nextPage(NormalMode, anEmptyAnswer).url
      }
    }
