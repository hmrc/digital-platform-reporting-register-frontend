  ...
  "IndividualEmailAddress Controller" - {
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        ...
        val viewModel = IndividualEmailAddressViewModel(NormalMode, aUserAnswers, form)
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      val userAnswers = minimalUserAnswers.set(IndividualEmailAddressPage, "foo@example.com").success.value
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        ...
        redirectLocation(result).value mustEqual IndividualEmailAddressPage.nextPage(NormalMode, minimalUserAnswers).url
      }
    }
    ...
    "must return a Bad Request and errors when invalid data is submitted" in {
      ...
      running(application) {
        ...
        val view = application.injector.instanceOf[IndividualEmailAddressView]
        val result = route(application, request).value
        val viewModel = IndividualEmailAddressViewModel(NormalMode, aUserAnswers, boundForm)
