...
class IndividualIdentityConfirmedControllerSpec extends SpecBase {
  ...
  "IndividualIdentityConfirmed Controller" - {
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        val request = FakeRequest(GET, routes.IndividualIdentityConfirmedController.onPageLoad().url)
        val result = route(application, request).value
        val view = application.injector.instanceOf[IndividualIdentityConfirmedView]
